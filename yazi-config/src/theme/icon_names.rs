use std::{ops::Deref, sync::Arc};

use arc_swap::ArcSwap;
use hashbrown::HashMap;
use serde::{Deserialize, Deserializer};
use yazi_codegen::Overlay;
use yazi_shim::arc_swap::IntoPointee;

use crate::Icon;

#[derive(Default, Overlay)]
pub struct IconNames(ArcSwap<HashMap<String, Icon>>);

impl Deref for IconNames {
	type Target = ArcSwap<HashMap<String, Icon>>;

	fn deref(&self) -> &Self::Target { &self.0 }
}

impl From<HashMap<String, Icon>> for IconNames {
	fn from(inner: HashMap<String, Icon>) -> Self { Self(inner.into_pointee()) }
}

impl IconNames {
	pub fn matches(&self, name: &str) -> Option<Icon> {
		let inner = self.0.load();
		inner.get(name).or_else(|| inner.get(&name.to_ascii_lowercase())).cloned()
	}

	pub(super) fn unwrap_unchecked(self) -> HashMap<String, Icon> {
		Arc::try_unwrap(self.0.into_inner()).expect("unique icon names arc")
	}
}

impl<'de> Deserialize<'de> for IconNames {
	fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
		deserialize_icon_names(deserializer).map(Self::from)
	}
}

pub(super) fn deserialize_icon_names<'de, D>(
	deserializer: D,
) -> Result<HashMap<String, Icon>, D::Error>
where
	D: Deserializer<'de>,
{
	#[derive(Deserialize)]
	struct Helper {
		name: String,
		#[serde(flatten)]
		icon: Icon,
	}

	Ok(
		Vec::<Helper>::deserialize(deserializer)?
			.into_iter()
			.map(|entry| (entry.name, entry.icon))
			.collect(),
	)
}
