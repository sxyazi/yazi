...
use super::{counters::Counter, filename_template::{ParseError, ParsedLine, TemplatePart}};
...
impl fmt::Display for PathGenError<'_> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { self.write_to(f) }
}

impl Error for PathGenError<'_> {}

impl PathGenError<'_> {
	/// Writes a formatted error message to a provided formatter or buffer.
	///
	/// # Arguments
	///
