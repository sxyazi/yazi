...
use yazi_shim::ratatui::LineIterBuilder;
...
pub(super) fn lines(lua: &Lua) -> mlua::Result<Value> {
	let f = lua.create_function(|lua, (s, opts): (mlua::String, Table)| {
		...
		let mut it = LineIterBuilder::default()
			.tab_size(opts.raw_get("tab_size")?)
			.width(opts.raw_get("width")?)
			.wrap(*opts.raw_get::<Wrap>("wrap")?)
			.ansi(opts.raw_get("ansi")?)
			.build(&s)
			.into_lua_err()?;
