...
/// Set cursor style
pub struct SetCursorStyle(pub u8);

impl Display for SetCursorStyle {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "\x1b[{} q", self.0) }
}
...
impl Display for RestoreCursorStyle {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		let style = match shape {
			2 if blink => CursorStyle::BlinkingUnderline,
			2 if !blink => CursorStyle::SteadyUnderline,
			3 if blink => CursorStyle::BlinkingBar,
			3 if !blink => CursorStyle::SteadyBar,
			_ if blink => CursorStyle::Default,
			_ if !blink => CursorStyle::SteadyBlock,
			_ => unreachable!(),
		};

		SetCursorStyle(style as u8).fmt(f)
	}
}
